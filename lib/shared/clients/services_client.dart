...
import 'package:estetica_beleza/models/tables/services.dart';
...
class ServiceClient {
  Future<List<Service>> consultarServices() async {
    String? mensagemErro;
    String? token = '';

    var resp = await CommomClient.getRequest(
      DotEnv.dotenv.env['BACKEND_URL_BASE']! + "/services/findAll",
      token,
    ...
    if (resp.statusCode == 200) {
      ...
      if (rr.statusCode == 200) {
        return rr.data.map<Service>((v) => Service.fromJsonApi(v)).toList();
      }
